...
let COVERAGE = flag("coverage", "catalog");
...
/* ---------- dev manifest ----------
 *
 * WHY extension/manifest.json EXISTS AGAIN
 *
 * v1 was a load-unpacked folder: point Chrome at extension/ and go. Generating
 * the manifest into dist/ broke that, and "run a build first" is a real cost
 * when the folder is what you demo from and what you have muscle memory for.
 *
 * So the build also writes a Chrome dev manifest back into extension/. It is
 * committed, so a fresh clone can load the folder with no build step at all.
 *
 * It is NOT the shipping artifact. dist/ still owns per-browser output --
 * Firefox's event page, Safari's trimmed permissions, the broad-coverage
 * variant. This one is chrome+catalog, which is what a demo needs.
 *
 * Regenerated on every build so it cannot drift from the catalog. If you edit
 * it by hand the next build overwrites you; edit sites.js instead.
 */
function writeDevManifest(catalog) {
  const saved = COVERAGE;
  COVERAGE = "catalog"; // dev manifest is always catalog coverage
  const m = buildManifest("chrome", catalog);
  COVERAGE = saved;

  m._comment = [
    "GENERATED by tools/build.mjs -- do not edit.",
    "",
    "This is the load-unpacked target for local development and demos:",
    "  chrome://extensions -> Developer mode -> Load unpacked -> extension/",
    "",
    "Edit extension/sites.js to change site coverage, then run `npm run build`.",
    "Hand edits here are overwritten.",
    "",
    "For production builds (Firefox event page, Safari, broad coverage) use",
    "dist/ -- see enterprise/README.md.",
  ];

  writeFileSync(join(SRC, "manifest.json"), JSON.stringify(m, null, 2) + "\n");
  console.log(`  ${"dev".padEnd(8)} ${"catalog".padEnd(8)} -> extension/manifest.json (load-unpacked target)`);
}
